...
export default function Restaurantpage() {
  ...
  const filteredRestaurants = restaurantsData.filter(res => {
    const cuisineMatch = selectedCuisine.length === 0 || selectedCuisine.includes(res.cuisine);
    const priceMatch = res.pricePerPerson >= priceRange[0] && res.pricePerPerson <= priceRange[1];
    return cuisineMatch && priceMatch;
  });

  const totalCost = guests * (selectedRestaurant?.pricePerPerson || 0);
        ...
        <div className="grid md:grid-cols-4 gap-6">
          ...
          <Card className="shadow">
            ...
            <CardContent className="space-y-6">
              <div>
                ...
                {["Indian","Italian","Chinese"].map(cuisine=>(
                  <div key={cuisine} className="flex gap-2">
                    <Checkbox checked={selectedCuisine.includes(cuisine)}
              ...
              <div>
                ...
                <Slider defaultValue={[300,1500]} min={300} max={1500} step={100}
          ...
          <div className="md:col-span-3 grid md:grid-cols-3 gap-6">
            {filteredRestaurants.map(res=>(
              <Card key={res.id} className="shadow hover:shadow-xl transition">
                ...
                <CardContent className="p-4 space-y-2">
                  ...
                  <p>⭐ {res.rating}</p>
                  <p>₹{res.pricePerPerson} / person</p>
                  <Button className="w-full mt-2"
                    onClick={()=>{setSelectedRestaurant(res);setOpenDialog(true);}}>
                    Book Table
                  </Button>
                </CardContent>
              </Card>
      ...
      <Dialog open={openDialog} onOpenChange={setOpenDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            {tablesLayout.map(table=>(
              <button key={table.id}
                ...
                ${selectedTable?.id===table.id?"bg-gray-900 text-white":"bg-white"}`}>
                Table {table.id}
          ...
          <div className="bg-gray-100 p-3 rounded mb-4">
            <p>Guests: {guests}</p>
            <p>Price per person: ₹{selectedRestaurant?.pricePerPerson}</p>
